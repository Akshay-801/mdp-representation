...
### State Space
The state space ($S$) represents the distinct grid positions the agent can occupy.$$S = \{0, 1, 2\}$$
...
### Action Space
The action space ($A$) consists of the directional movements the agent can take from the non-terminal state.$$A = \{\text{Left}, \text{Right}\}$$
